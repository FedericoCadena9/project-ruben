...
const Home = ({ projects, meta, categorias, pageSize, jwt }) => {
  ...
  // Función que retornará los datos establecidos para los Filtros
        ...
        <div className="ml-0 transition md:ml-64 flex flex-col flex-grow duration-150 ease-in">
          ...
          <div className="pt-20 sm:pt-0">
            <div className="flex flex-wrap items-center justify-center  gap-4 mx-8">
              ...
              {status === "success" &&
                data.map((item, i) => (
                  <ProjectCard key={i} {...item.attributes} />
                ))}
            </div>

            {/* Paginación */}
            <div className="relative z-0 flex -space-x-px pb-10 items-center justify-center mt-12">
              <button
                type="button"
...
export const getServerSideProps = async (ctx) => {
  ...
  const projects = await dataApi(`${baseUrl}/detalle-proyectos`, jwt);

  // const categorias = await dataApi(`${baseUrl}/detalle-proyectos`);

  return {
    props: {
      projects: projects.data,
      meta: projects.meta.pagination,
      pageSize,
      jwt,
    },
  };
